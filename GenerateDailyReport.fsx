...
|> fun game ->
    $"\n## Connections\n\n**By:** %s{game.Info.Editor}\n\n**Categories:**\n"
    |> toStringBuilder
    game.Categories |> List.iteri (fun i c ->
        $"%d{i+1}. **%s{c.Title}**" |> toStringBuilder
        c.Cards |> List.iter (fun card ->
            $"    - `%s{card}`" |> toStringBuilder
        )
    )
...
|> fun game ->
    ...
    $"**Spangram:** `%s{game.Spangram}`\n\n**Theme words:**\n"
        |> toStringBuilder
    game.ThemeWords |> List.iter (fun word ->
        $"- `%s{word}`" |> toStringBuilder
    )
...
|> fun game ->
    ...
    if not (List.isEmpty game.Info.Constructors) then
        "**Constructed by:**\n" |> toStringBuilder
        game.Info.Constructors |> List.iter (fun word ->
            $"- %s{word}" |> toStringBuilder
        )
    $"\n**Solution:**\n"
        |> toStringBuilder
    game.Solution |> Array.map (fun row ->
        row |> Array.map (
            function
